...
public class SwitchName extends ModifiableViewName {
  ...
  public String runRule(AxeProps axeProps) {
    String visibleText = axeProps.get(AxeProps.Name.VISIBLE_TEXT, String.class);
    final String labeledBy = axeProps.get(AxeProps.Name.LABELED_BY, String.class);

    if (visibleText.equalsIgnoreCase("on") || visibleText.equalsIgnoreCase("off")) {
      visibleText = null;
    }

    if ((AxeTextUtils.isNullOrEmpty(labeledBy) && (AxeTextUtils.isNullOrEmpty(visibleText))) ||
